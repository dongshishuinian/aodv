			   ...
			   u_int32_t dest_seqno, struct in_addr orig_addr)
{
    ...
    log_pkt_fields((AODV_msg *) rreq);
                ...
                while(temp!=NULL)
                {
                   if(temp->d_addr == dest_addr.s_addr )
                   {
                        haveRecord = 1;
                        break;
                   }
                ...
                if(haveRecord==0)
                {
                   temp = (lostDes*)malloc(sizeof(lostDes));
                   if(temp!=NULL)
                   {
                      temp->next = g_lostDes->next;
                      temp->d_addr = dest_addr.s_addr;
		     // memset(&temp->last_send_time,0,sizeof(struct timeval));
                      g_lostDes->next = temp;
                      printf("===route %s(%u)  not found,added in the var===\n",ip_to_str(dest_addr),temp->d_addr);
                   }
                }
