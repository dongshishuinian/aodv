...
ARM_LD=arm-linux-gnueabihf-gcc-ld
MIPS_CC=mipsel-linux-gcc
MIPS_LD=mipsel-linux-gcc-ld
...
export CC ARM_CC MIPS_CC arm_compile
...
CFLAGS=-fgnu89-inline $(OPTS) $(DEBUG) $(DEFS) $(XDEFS)
LD_OPTS=
...
#kaodv-arm:
#	$(MAKE) -C $(AODVDIR)/lnx KERNEL_DIR=$(KERNEL_ARM_DIR) KCC=$(ARM_CC) LD=$(ARM_LD) XDEFS=$(XDEFS) kaodv-arm
kaodv-arm:
	cd lnx && $(MAKE)
