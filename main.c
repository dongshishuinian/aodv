...
void init_glostDes()
{
    g_lostDes = (lostDes*)malloc(sizeof(lostDes));
    if(g_lostDes==NULL)return;
    g_lostDes->next = NULL;
    g_lostDes->d_addr = (in_addr_t)(0xFFFFFFFF);
//    memset(&g_lostDes->last_send_time,0,sizeof(struct timeval));
}
void cleanup_glostDes()
{
    free(g_lostDes);
}
